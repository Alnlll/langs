...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    fg.axes2d()
        .set_title("Cat breed classfication result", &[])
        .set_legend(Graph(0.9), Graph(0.1), &[], &[])
        .set_x_label("height (cm)", &[])
        .set_y_label("length (cm)", &[])
        .points(
            &x[0], &y[0], &[Caption("Cat breed 1"), Color("red"), PointSymbol('+')],)
        .points(
            &x[1], &y[1], &[Caption("Cat breed 2"), Color("green"), PointSymbol('x')],)
        .points(
            &x[2], &y[2], &[Caption("Cat breed 3"), Color("blue"), PointSymbol('o')],);

        
    fg.save_to_png("./cat_breed.png", 1280, 720);
